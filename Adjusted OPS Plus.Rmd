...
combined_files_all_years <- multi_union_all("/Users/maxhoff/Downloads/Adjusted-OPS-Plus-Part-1-Creating-Handedness-Ball-Park-Factors-main/data/play_by_play")
```
...
play_key <- unique(read_csv("/Users/maxhoff/Downloads/Adjusted-OPS-Plus-Part-1-Creating-Handedness-Ball-Park-Factors-main/data/play_key_v2.csv"))
```
...
rosters_all_years <- distinct(multi_union_all("/Users/maxhoff/Downloads/Adjusted-OPS-Plus-Part-1-Creating-Handedness-Ball-Park-Factors-main/data/rosters_all_years"))
```
...
plays_all_years <- merge(plays_all_years, 
                         rosters_all_years[,c("batter_id", "bats")], 
                         by = "batter_id", 
                         all.x = TRUE)
```
...
bpf_ops <- bpf_ops %>%
  mutate(SLG_dif = SLG_home - SLG_away,
         OBP_dif = OBP_home - OBP_away,
         OPS_dif = (SLG_home + OBP_home) - (SLG_away + OBP_home)) %>%
  select(-SLG_away, -OBP_away, -SLG_home, -OBP_home)
```

```{r}
##part 2
##now onto creating summary stats at the player level for the 2019 season

##home stats df
plays_all_years %>%
  group_by(batter_id, `home/away`, bats) %>% 
  summarize(SLG = (sum(TB, na.rm = TRUE) / sum(AB, na.rm = TRUE)),
  ...
  mutate() %>%
  select(-,-) %>%
##redo process before based on these changes
